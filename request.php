...
class JoinRequestHandler {
    ...
    private function processCaptchaResponse($request, $userAnswer) {
        if ($userAnswer === $request['captcha_answer']) {
            ...
            $reply .= "Теперь ответьте на несколько вопросов(ответы подтверждают согласие на краткосрочное ознакомление с персональными данными, после вступления\отказа в вступлении в группу персональные данные будут удалены, собираться, храниться и обрабатываться не будут):\n\n";
